...
{
    public class FindDrink : JobTask
    {
        Tile? fridge = null;
        Item? item = null;
        Tile? sink = null;
        ...
        public override void Action()
        {
            ...
            if (Owner_Character.Stats.Thirst == 0)
            {
                //We don't need a drink anymore
            ...
            if (item != null)
            {
                if (WorldUtil.NextTo(fridge.Location, Owner_Character.Location))
                {
                    ...
                    else
                    {
                            ...
                            fridge.Texture.Name.Contains("Used"))
                        {
                            ...
                            int seconds = Owner_Character.Stats.Thirst >= 60 ? 10 : 20;
                            ...
                            seconds = Owner_Character.Stats.Thirst >= 60 ? 1 : 2;
                        ...
                        {
                            int seconds = Owner_Character.Stats.Thirst >= 60 ? 1 : 2;
                ...
                {
                    PathTo(fridge.Location);
                }
            }
            else if (sink?.Location != null)
            {
                if (WorldUtil.NextTo(sink.Location, Owner_Character.Location))
                {
                    Direction direction = WorldUtil.GetDirection(Owner_Character.Location, sink.Location);
                    if (direction != Owner_Character.Direction)
                    {
                        SubTasks.Add(new Turn
                        {
                    ...
                    {
                        TimeSpan duration = TimeSpan.FromMilliseconds(Owner_Character.Stats.Thirst * 1000);

                        SubTasks.Add(new UseSink
                        {
                            Name = "UseSink",
                            Owner_Character = Owner_Character,
        ...
        private void PathTo(Location target)
        {
            if (Owner_Character?.Location == null ||
                TimeManager.Now == null)
            {
            ...
            {
                if (Owner_Character.Stats.Thirst >= 60)
                {
            ...
            else if (bottom_tiles != null)
            {
                ...
                List<ALocation> path = Pathing.GetPath(bottom_tiles, middle_tiles, Owner_Character, target, distance, true);
